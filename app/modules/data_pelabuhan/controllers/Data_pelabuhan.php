...
class Data_pelabuhan extends CI_Controller
{
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $person) {
            $provinsi = $this->db->where('id', $person->provinsi_asal)->get('wilayah_provinsi')->row();
            $no++;
            $row = array();
            $row[] = $no;
            $row[] = $person->nama_pelabuhan;
            if (isset($provinsi->nama)) {
                $row[] = $provinsi->nama;
            } else {
                $row[] = "Deleted Data Provinsi";
            }
